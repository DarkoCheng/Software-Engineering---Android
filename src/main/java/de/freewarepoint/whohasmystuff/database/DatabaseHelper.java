	...
	private static String convertDatabaseToXml(OpenLendDbAdapter database) throws ParseException {
		...
		if (c.getCount() > 0) {
			...
			while (!c.isAfterLast()) {
                ...
                String lenderNum = c.getString(c.getColumnIndexOrThrow(OpenLendDbAdapter.KEY_LENDER_PHONE_NUM));
                sb.append(" lenderNum=\"").append(replace(lenderNum)).append("\"");

                String lendeeNum = c.getString(c.getColumnIndexOrThrow(OpenLendDbAdapter.KEY_LENDEE_PHONE_NUM));
                sb.append(" lendeeNum=\"").append(replace(lendeeNum)).append("\"");
