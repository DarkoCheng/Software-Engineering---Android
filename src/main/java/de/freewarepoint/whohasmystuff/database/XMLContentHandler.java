...
public class XMLContentHandler extends DefaultHandler {
    ...
    private void parseLentObjectAttributes(Attributes attributes) {
        ...
        for (int i = 0; i < attributes.getLength(); i++) {
            ...
            else if("lenderNum".equals(name))
            {
                lentObject.lenderPhoneNum = attributes.getValue(i);
            }
            else if("lendeeNum".equals(name))
            {
                lentObject.lendeePhoneNum = attributes.getValue(i);
            }
        }
